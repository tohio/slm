...
Usage:
    python -m slm.config_gen --stage pretrain --gpu h200 --size 125m --gpus 1
    python -m slm.config_gen --stage sft      --detect    --size 350m --gpus 4
    python -m slm.config_gen --stage dpo      --gpu b200  --size 1b   --gpus 8 --mode aggressive
...
# SFT chat profiles — values come from the hand-tuned configs.
# state_gb matches pretrain (same architecture, same optimizer).
...
def _compute_sft_config_with_profile(
    ...
    # Activation estimates are analytical — flag for future measurement
    warns.append(
        ...
        f"SFT_{sft_variant.upper()}_PROFILES['{size}'] in slm/config_gen.py."
    )
...
def compute_dpo_config(gpu_key: str, size: str, num_gpus: int,
    ...
    warns.append(
        ...
        f"DPO_PROFILES['{size}'] in slm/config_gen.py."
    )
...
def _yaml_header(cfg: GeneratedConfig) -> str:
    ...
    lines = [
        "# ─────────────────────────────────────────────────────────────────────────────",
        "# Auto-generated by slm/config_gen.py",
        f"# Stage:      {cfg.stage}",
